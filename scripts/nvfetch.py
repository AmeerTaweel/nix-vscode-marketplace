...
if not Path(init_json).exists():
    print(
        f"Error: file {init_json} was selected for initialization, but it doesn't exist"
    )
    sys.exit()

if not Path(init_nix).exists():
    print(
        f"Error: file {init_nix} was selected for initialization, but it doesn't exist"
    )
    sys.exit()
